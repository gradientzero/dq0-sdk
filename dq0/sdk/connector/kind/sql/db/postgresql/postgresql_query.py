...
class PostgreSQLQuery(Query):
    def __init__(self, connector, query_string):
        ...
        if not connector.get_kind() == ConnectorKind.SQL_POSTGRESQL:
            raise ValueError(f"connector with kind={connector.get_kind()} is not of valid {ConnectorKind.SQL_POSTGRESQL} kind")
