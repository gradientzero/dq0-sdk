...
class QueryProcessorOpenDP:
    ...
    def verify(attribute, role_uuids=None):
        ...
        QueryProcessorOpenDP.verify_attributes(attributes=attribute.get_value(), role_uuids=role_uuids)
    ...
    def verify_attributes(attributes, role_uuids=None):
        ...
        if type_name_attributes[0].get_value() != 'opendp':
            raise Exception(f"query processor type_name value {type_name_attributes[0].get_value()} does not match 'opendp'")
