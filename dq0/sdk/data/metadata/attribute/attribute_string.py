...
class AttributeString(Attribute):
    ...
    def __str__(self, request_uuids=set()):
        ...
        if super_str is None:
            return None
        return super_str + ' ' + MetaUtils.str_from(self.value, quoted=True).replace('\n', "\n  ")
    ...
    def is_mergeable_with(self, other, overwrite_value=False, overwrite_permissions=False, request_uuids=set(), explanation=None):
        if not super().is_mergeable_with(other=other, overwrite_value=overwrite_value, overwrite_permissions=overwrite_permissions, request_uuids=request_uuids, explanation=explanation):
            Explanation.dynamic_add_message(explanation=explanation, message=f"AttributeString.is_mergeable_with(...): super() is not mergeable")            
            return False
        if not overwrite_value and self.value != other.value:
            Explanation.dynamic_add_message(explanation=explanation, message=f"AttributeString.is_mergeable_with(...): value differs without overwrite_value")            
            return False
        if self.value != other.value and not Permissions.is_allowed_with(permissions=self.permissions, action=Action.WRITE_VALUE, request_uuids=request_uuids, explanation=explanation):
            Explanation.dynamic_add_message(explanation=explanation, message=f"AttributeString.is_mergeable_with(...): value differs without write_value permissions")            
            return False
        return True
