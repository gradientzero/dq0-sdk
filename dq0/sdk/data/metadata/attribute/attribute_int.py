...
class AttributeInt(Attribute):
    ...
    def is_mergeable_with(self, other, overwrite_value=False, overwrite_permissions=False, request_uuids=[], explanation=None):
        ...
        if not overwrite_value and self.value != other.value:
            Explanation.dynamic_add_message(explanation=explanation, message=f"AttributeInt.is_mergeable_with(...): value differs without overwrite_value")            
            return False
        if self.value != other.value and not Permissions.is_allowed_with(permissions=self.permissions, action=Action.WRITE_VALUE, request_uuids=request_uuids, explanation=explanation):
            Explanation.dynamic_add_message(explanation=explanation, message=f"AttributeInt.is_mergeable_with(...): value differs without write_value permissions")            
            return False
        return True
