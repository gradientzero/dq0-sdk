...
class Attribute:
    ...
    def check_list(attribute_list, allowed_keys_type_names_permissions=None):
        ...
        if 0 < regular_key_count and 1 < none_key_count:
            raise Exception(f"may only have single none (null) key in list with regular keys, there is {regular_key_count} regular key(s) and {none_key_count} none key(s)")
        ...
        for attribute_a in attribute_list_a:
            for attribute_b in attribute_list_b:
                if attribute_a.key == attribute_b.key and not attribute_a.is_merge_compatible_with(other=attribute_b, explanation=explanation):
                    Explanation.dynamic_add_message(explanation=explanation, message=f"Attribute.are_merge_compatible(...): attributes[{attribute_a.type_name}] with matching key {attribute_a.key} are not compatible")
                    return False
        return True
        ...
        for elem_a in attribute_list_a:
            ...
            for elem_b in attribute_list_b:
                if elem_a.is_merge_compatible_with(other=elem_b, explanation=None):
                    if not elem_a.is_mergeable_with(other=elem_b, overwrite_value=overwrite_value, overwrite_permissions=overwrite_permissions, request_uuids=request_uuids, explanation=explanation):
                        Explanation.dynamic_add_message(explanation=explanation, message=f"Attribute.are_mergeable(...): compatible attributes[{elem_a.type_name}] with key {elem_a.key} are not mergeable")
                        return False
                    else:
                        if found_match:
                            Explanation.dynamic_add_message(explanation=explanation, message=f"Attribute.are_mergeable(...): duplicate mergeable attributes[{elem_a.type_name}] with key {elem_a.key} are not allowed")
                            return False
                        found_match = True
        return True
    ...
    def is_merge_compatible_with(self, other, explanation=None):
        if not isinstance(other, Attribute):
            Explanation.dynamic_add_message(explanation=explanation, message=f"Attribute[{self.type_name}].is_merge_compatible_with(...): other is not of type Attribute, is of type {type(other)} instead")
            return False
        if self.type_name != other.type_name:
            Explanation.dynamic_add_message(explanation=explanation, message=f"Attribute[{self.type_name}].is_merge_compatible_with(...): type_name mismatch {self.type_name} != {other.type_name}")
            return False
        if self.key != other.key:
            Explanation.dynamic_add_message(explanation=explanation, message=f"Attribute[{self.type_name}].is_merge_compatible_with(...): key mismatch {self.key} != {other.key}")
            return False
        if not Permissions.are_merge_compatible(permissions_a=self.permissions, permissions_b=other.permissions, explanation=explanation):
            Explanation.dynamic_add_message(explanation=explanation, message=f"Attribute[{self.type_name}].is_merge_compatible_with(...): permissions are not compatible")
            return False
        return True
    ...
    def is_mergeable_with(self, other, overwrite_value=False, overwrite_permissions=False, request_uuids=[], explanation=None):
        ...
        if not Permissions.are_mergeable(permissions_a=self.permissions, permissions_b=other.permissions, overwrite=overwrite_permissions, explanation=explanation):
            Explanation.dynamic_add_message(explanation=explanation, message=f"Attribute[{self.type_name}].is_mergeable_with(...): permissions are not mergeable")            
            return False
        ...
        if self.permissions != merged_permissions and not Permissions.is_allowed_with(permissions=self.permissions, action=Action.WRITE_PERMISSIONS, request_uuids=request_uuids, explanation=explanation):
            Explanation.dynamic_add_message(explanation=explanation, message=f"Attribute[{self.type_name}].is_mergeable_with(...): writing permissions without write_permissions permission is not allowed")            
            return False
        return True
