...
class AttributeBoolean(Attribute):
    def __init__(self, key, value):
        super().__init__(type_name=AttributeType.TYPE_NAME_BOOLEAN, key=key)
        if value is None:
            raise Exception("value is None")
        if not isinstance(value, bool):
            raise Exception("value is not of type boolean")
        self.value = value

    def __str__(self):
        return super().__str__() + ' ' + MetaUtils.str_from(self.value)

    def __repr__(self):
        return "AttributeBoolean(key=" + MetaUtils.repr_from(self.key) + ", value=" + MetaUtils.repr_from(self.value) + ')'

    def copy(self):
        return AttributeBoolean(key=self.key, value=self.value)
    ...
    def is_mergeable_with(self, other, overwrite=False):
        if not super().is_mergeable_with(other=other, overwrite=overwrite):
            # print(f"super not mergeable <-- AttributeBoolean.is_mergeable_with:(self={self} other={other} overwrite={overwrite})")
            return False
        if not overwrite and self.value != other.value:
            # print(f"value mismatch on no overwrite <-- AttributeBoolean.is_mergeable_with:(self={self} other={other} overwrite={overwrite})")
            return False
        return True
