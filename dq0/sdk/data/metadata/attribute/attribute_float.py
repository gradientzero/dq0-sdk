...
class AttributeFloat(Attribute):
    def __init__(self, key, value):
        super().__init__(type_name=AttributeType.TYPE_NAME_FLOAT, key=key)
        if value is None:
    ...
    def __repr__(self):
        return "AttributeFloat(key=" + MetaUtils.repr_from(self.key) + ", value=" + MetaUtils.repr_from(self.value) + ')'

    def copy(self):
        return AttributeFloat(key=self.key, value=self.value)
