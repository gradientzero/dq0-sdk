        ...
        for node_table in metadata.node.child_nodes[0].child_nodes[0].child_nodes:
            for node_column in node_table.child_nodes:
                ...
                if attribute_column_name is not None:
                    ...
                    attribute_column_is_feature = attribute_column_machine_learning.get_attribute(key='is_feature') if attribute_column_machine_learning is not None else None
                    ...
                    attribute_column_is_target = attribute_column_machine_learning.get_attribute(key='is_target') if attribute_column_machine_learning is not None else None
    ...
    def get_col_types(metadata):
        ...
        for node_table in metadata.node.child_nodes[0].child_nodes[0].child_nodes:
            for node_column in node_table.child_nodes:
                ...
                attribute_column_name = attribute_column_data.get_attribute(key='name') if attribute_column_data is not None else None
                if attribute_column_name is not None:
                    attribute_column_data_type_name = attribute_column_data.get_attribute(key='data_type_name') if attribute_column_data is not None else None
