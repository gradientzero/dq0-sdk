...
class MetaSectionColumnPrivacyFloat(MetaSection):
    @staticmethod
    def fromYamlDict(yaml_dict):
        ...
        if type_name != MetaSection.TYPE_NAME_COLUMN_PRIVACY_FLOAT:
            raise Exception(f"type_name must be {MetaSection.TYPE_NAME_COLUMN_PRIVACY_FLOAT} was {type_name}")
        ...
        return MetaSectionColumnPrivacyFloat(name, bounded, lower, upper, use_auto_bounds, auto_bounds_prob, auto_lower, auto_upper, discrete, min_step, synthesizable)
    ...
    def __init__(
            ...
            min_step=1.0,
            synthesizable=True):
        super().__init__(MetaSection.TYPE_NAME_COLUMN_PRIVACY_FLOAT, name)
