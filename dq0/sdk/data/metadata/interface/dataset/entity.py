...
class Entity:
    ...
    def get_child_entity(self, name=None, index=-1):
        if name is None and index < 0:
            index = 0
        if name is None:
            child_node = self._node.get_child_node(index=index) if self._node is not None else None
            if child_node is None:
                raise Exception("index based get may only work for existing child nodes")
            name = Entity.name_of(node=child_node)
        else:
            child_node = self._node.get_child_node(attributes_map={'data': {'name': name}}) if self._node is not None else None
    ...
    def add_child_node(self, child_node):
        self.create()
        self._node.add_child_node(child_node=child_node)
    ...
    def remove_child_node(self, name=None, index=-1):
        ...
        if name is None:
            if index < 0:
                index = 0
            child_node = self._node.get_child_node(index=index)
            if child_node is None:
                return
            name = Entity.name_of(node=child_node)
        self._node.remove_child_node(index=index, attributes_map={'data': {'name': name}})
    ...
    def remove_child_nodes(self, attributes_map=None):
        if self._node is None:
            return
        child_nodes = self._node.get_child_nodes(attributes_map=attributes_map)
    ...
    def get_attribute_group(self, key):
        if key not in self._attribute_groups:
            attribute_list = self._node.get_attribute(key=key) if self._node is not None else None
    ...
    def set_name(self, old_name, new_name):
        ...
        if self._name != old_name:
            raise Exception(f"name mismatch: {self._name} != {old_name}")
        self._name = new_name
    ...
    def add_attribute(self, attribute):
        self.create()
        self._node.add_attribute(attribute=attribute)

    def remove_attribute(self, key):
        if self._node is None:
            return
        self._node.remove_attribute(key=key)
