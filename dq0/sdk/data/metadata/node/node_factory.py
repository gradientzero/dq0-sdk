...
class NodeFactory:
    ...
    def get_node_yaml_type(yaml_content, format_type=None):
        if isinstance(yaml_content, list):
            if format_type is not None:
                raise Exception("when the yaml content is a list, the format type may not be given")
    ...
    def from_yaml_dict(yaml_dict):
        ...
        attributes = [AttributeFactory.from_yaml_dict(yaml_dict=attribute_yaml_dict) for attribute_yaml_dict in
                      attributes_yaml_list] if attributes_yaml_list is not None else None
    ...
    def from_yaml_simple(yaml_simple, force_list=False):
        ...
        while key in yaml_simple:
            ...
            attributes = AttributeFactory.from_yaml_simple(yaml_simple_key=None, yaml_simple_value=attributes_yaml_content) \
                if attributes_yaml_content is not None else None
            child_nodes_yaml_content = yaml_simple_dict.pop('child_nodes', None)
            child_nodes = NodeFactory.from_yaml_content(yaml_content=child_nodes_yaml_content, force_list=True) \
                if child_nodes_yaml_content is not None else None
