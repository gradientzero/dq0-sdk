...
class CSV(Source):
    ...
    def __init__(self, path, use_original_header=None, header_row=None, header_columns=None, feature_cols=None, target_cols=None):
        super().__init__(path)
        self.type = 'csv'
    ...
    def read(self, **kwargs):
        ...
        names = None
        skiprows = []
        if kwargs is not None and "skiprows" in kwargs.keys():
            skiprows = kwargs.pop("skiprows", [])
        if self.use_original_header:
            header = self.header_row
            try:
                df = pd.read_csv(self.path, header=header, **kwargs)
            except:
                print('Failed to load CSV with use_original_header {}, '
                    'header_row {} and kwargs {}'.format(
                    self.use_original_header, self.header_row, kwargs))
        else:
