...
class LogisticRegressionDP(ClassifierMixin, Estimator):
    ...
    def __init__(self, target_epsilon=1, data_norm=None, tol=1e-4, C=1.0, fit_intercept=True, max_iter=100, verbose=0,
...
class LinearRegressionDP(RegressorMixin, Estimator):
    ...
    def __init__(self, target_epsilon=1, bounds_X=None, bounds_y=None, fit_intercept=True, copy_X=True, accountant=None,
                 **kwargs):
        """ Note the diff priv models are DP only. The target_epsilon value has to be set on initialize."""
        super().__init__(**kwargs)
        self.model_type = 'DPLinearModelEstimatorRegressor'
        self.model = linear_regression.LinearRegression(epsilon=target_epsilon, bounds_X=bounds_X, bounds_y=bounds_y, fit_intercept=fit_intercept,
