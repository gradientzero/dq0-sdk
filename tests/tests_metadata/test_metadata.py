...
'''
    ...
    # load metadata
    metadata, _, _ = Metadata.from_yaml_file(filename='test.yaml', role_uuids=role_uuids)
    ...
    attribute_name_dct = None
    for attribute_dct in metadata_sm_dct['meta_dataset']['node']['attributes'] if 'attributes' in metadata_sm_dct['meta_dataset']['node'] else []:
        if attribute_dct['key'] == 'data':
            for sub_attribute_dct in attribute_dct['value']:
                if sub_attribute_dct['key'] == 'name':
                    attribute_name_dct = sub_attribute_dct
